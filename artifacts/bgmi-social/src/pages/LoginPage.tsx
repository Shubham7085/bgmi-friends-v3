...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg: string = err?.code || err?.message || 'Invalid credentials';
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <motion.button
            type="submit"
            disabled={loading}
            className="w-full py-3 rounded-xl btn-primary text-sm font-medium flex items-center justify-center gap-2 disabled:opacity-60"
            whileTap={{ scale: 0.98 }}
          >
            {loading ? (
